...
## 2024-04-03 - [Action UI Slots Accessibility]
**Learning:** For action UI slots like hotbars or skills, using simple `div` elements visually works but limits accessibility and discoverability. The lack of proper keyboard interaction and missing shortcut hints can make navigation confusing, particularly for users reliant on keyboard shortcuts or screen readers.
**Action:** Always use semantic `<button>` elements for UI action slots, include explicit `aria-label`s for screen readers, and provide explicit keyboard shortcuts in the `title` attribute to enhance natural discoverability and accessible usage.
