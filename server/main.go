...
const MaxChatLength = 200 // Prevent excessively long chat messages
			...
			if player, ok := hub.players[playerID]; ok {
				...
				case "mob_hit":
					...
					if mob, ok := hub.MobManager.Mobs[mobID]; ok {
						...
						// and removing math.Sqrt by comparing squared distances.
						dx := mob.X - player.X
						dz := mob.Z - player.Z
				...
				case "ability_hit":
					...
					if mob, ok := hub.MobManager.Mobs[mobID]; ok {
						...
						hub.MobManager.mutex.Unlock()
						// Max Range needed.
						if distSq > 150.0*150.0 { // Generous range for now
							continue
						}
					} else {
						hub.MobManager.mutex.Unlock()
				...
				case "admin_action":
					...
					case "chat":
						...
						// Truncate to MaxChatLength using runes to avoid splitting multi-byte UTF-8 characters (like emojis)
